...
This is **v3.0.12**, a settings-only update-control patch on top of the v3.0.11 update-button baseline. The proven engine (streaming, model catalog, generated-file parsing, Free Endpoint model handling, and the Worker) was kept; the app was patched in place without a rewrite.
...
```
https://wigglez-sudo.github.io/nvidia-ai-desktop/?v=3.0.12
```
...
The Worker source is still a single file: **`worker/index.js`**. Its behaviour is unchanged in v3.0.12. **No Worker change is required for this patch** unless you want to redeploy the included copy for consistency.
...
After uploading, open the site with `?v=3.0.12` and check:
...
4. Sidebar name/status (bottom-left) opens the **Diagnostics** panel; version badge shows `v3.0.12`.
