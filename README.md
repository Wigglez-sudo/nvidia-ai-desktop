...
This is **v3.1.2**, a vision attachment patch on top of the v3.1.0 UI clarity baseline and v3.1.1 iOS keyboard fix. The proven engine (streaming, model catalog, generated-file parsing, Free Endpoint model handling, plugins, and the Worker) was kept; the app was patched in place without a rewrite.
...
```
https://wigglez-sudo.github.io/nvidia-ai-desktop/?v=3.1.2
```
...
The Worker source is still a single file: **`worker/index.js`**. Its behaviour is unchanged in v3.1.2. **No Worker change is required for this patch** unless you want to redeploy the included copy for consistency.
...
After uploading, open the site with `?v=3.1.2` and check:
...
4. Sidebar name/status (bottom-left) opens **App Status & Tools**; version badge shows `v3.1.2`.
